...
# TODO add update list to tooltip
# they must be on the same line, separated by a "\n"
...
if [ "$updates_arch" -gt 0 ]; then
    ...
    if [ "$updates_aur" -gt 0 ]; then ## TODO fix this hacky logic
        list_updates+="\n"
    fi
fi
...
# -------------------------------------------------------
## TODO fix HTML tag, something about escaping
updates=$(("$updates_arch" + "$updates_aur"))
tooltip="Update the system\n<span size=\\\"small\\\">${updates} packages</span>:\n${list_updates}"
